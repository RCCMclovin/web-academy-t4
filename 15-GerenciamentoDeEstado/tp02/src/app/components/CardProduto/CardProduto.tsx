...
export default function CardProduto({
  ...
  const { addFavorito, isFavorito, isAddFavoritoPending } = useFavoritosContext();
  const ehFavorito = () => isFavorito(produto.id);
  const [newFavorito, setNewFavorito] = useState<Produto | null>(null);
  const favoritar = () => {
    addFavorito(produto);
    setNewFavorito(produto);
  }

  useEffect(() => {
    if (!isAddFavoritoPending) setNewFavorito(null);
   },[isAddFavoritoPending, setNewFavorito])
        ...
        <div className="card-body bg-ligth">
          ...
          {mostrarBotao ? (
            <button
              ...
              type="button"
              onClick={() => favoritar()}
              disabled={ehFavorito() || (isAddFavoritoPending && newFavorito===produto)}
            >
              {(isAddFavoritoPending && newFavorito === produto) ? "Favoritando..." :
